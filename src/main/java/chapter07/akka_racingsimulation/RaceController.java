...
public class RaceController extends AbstractBehavior<RaceController.Command> {
    ...
    public static Behavior<Command> create() {
        return Behaviors.setup(context -> {
            return new RaceController(context);
        });
    }
    ...
    public Receive<Command> raceCompleteMessageHandler() {
        return newReceiveBuilder()
                .onMessage(GetPositionsCommand.class, message -> {
                    for(ActorRef<Racer.Command> racer : currentPositions.keySet()) { // destroy all child actors
                        getContext().stop(racer); // this will terminate the child actor
                    }
                    displayResults();
                    // cancel the timer
                    return Behaviors.withTimers(timers -> {
                        timers.cancelAll(); // this will cancel all the timers within this actor
                        return Behaviors.stopped(); // this will shutdown this actor
                    });
                })
                .build();
    }
